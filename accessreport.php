 <?php
    ...
    $count_res = mysqli_query($mysqli, $count_sql) or
    ...
    $user_agent_sql = "SELECT DISTINCT user_agent, count(user_agent) AS
 ua_count FROM access_tracker GROUP BY user_agent
 ORDER BY ua_count desc";
    $user_agent_res = mysqli_query($mysqli, $user_agent_sql)
 ...
 <li><em>accesses per browser: " . $user_agent_count . "</em>
    ...
    $page_title_sql = "SELECT DISTINCT page_title, count(page_title) AS
 pt_count FROM access_tracker GROUP BY page_title
 ORDER BY pt_count desc";
    $page_title_res = mysqli_query($mysqli, $page_title_sql)
        or die(mysqli_error($mysqli));
